...
class ApiCall {
        ...
        call.enqueue(object : Callback<EpisodesListDataModel> {
            ...
            override fun onResponse(
                ...
                if (response!!.isSuccess) {
                    ...
                    val episodes: EpisodesListDataModel = response.body() as EpisodesListDataModel
    ...
    fun getAllCharacters(callback: (CharactersListDataModel) -> Unit /* page: Int, count: Int*/) {
        ...
        // Use the enqueue() method of the Call object to
        // make an asynchronous API request.
        call.enqueue(object : Callback<CharactersListDataModel> {
            // This is an anonymous inner class that implements the Callback interface.
            override fun onResponse(
                ...
                if (response!!.isSuccess) {
                    // If the response is successful, parse the
                    // response body to a DataModel object.
                    val episodes: CharactersListDataModel = response.body() as CharactersListDataModel
                    // Call the callback function with the DataModel
                    // object as a parameter.
    ...
    suspend fun getSingleCharacter(characterId: String): CharacterDataModel =
        withContext(Dispatchers.IO) {
            ...
            return@withContext if (response != null && response.isSuccess && response.body() != null) {
                ...
                Log.d("loadin", "Error on getSingleCharacter")
            ...
            /*        call.enqueue(object : Callback<CharacterDataModel> {
                        // This is an anonymous inner class that implements the Callback interface.
                        override fun onResponse(response: Response<CharacterDataModel>?, retrofit: Retrofit?) {
                            if (response != null && response.isSuccess && response.body() != null) {
                                val character: CharacterDataModel = response.body()!!
                                callback(character)
                                Log.d("loadin", "Callback executed in getSingleCharacter")
                            } else {
                                Log.d("loadin", "Error on getSingleCharacter")
                                // Handle unsuccessful response or null body
                            }
